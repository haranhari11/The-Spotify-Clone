...
export const fetchPlaylistsMenu = (userId, accessToken) => {
  return (dispatch) => {
    ...
    fetch(request)
      ...
      .then((res) => {
        let playlistIds = res.items.map((item) => {
          return item.id;
        });
        dispatch(setPlaylistIds(playlistIds.toString()));
...
export const fetchPlaylistSongs = (userId, playlistId, accessToken) => {
  return (dispatch) => {
    const request = new Request(
      `https://api.spotify.com/v1/users/${userId}/playlists/${playlistId}/tracks?limit=50`,
      {
        headers: new Headers({
          Authorization: 'Bearer ' + accessToken,
        }),
...
export const setPlaylistIds = (playlistIds) => {
  return {
